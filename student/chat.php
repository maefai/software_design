...
<head>
    ...
    <title>Student Chat - GreenBridge</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Contact List Sidebar - Glassmorphism style */
        .chat-sidebar {
            flex: 0 0 350px;
        ...
        /* Chat Main Box - Glassmorphism style */
        .chat-main {
            flex: 1;
            background: rgba(255, 255, 255, 0.85);
            backdrop-filter: blur(10px);
...
<div class="chat-container">
    ...
    <div class="chat-main" id="chat-main">
        <div class="chat-empty" id="chat-empty">
            ...
            <p>Select a contact from the sidebar to view history or start sending real-time messages.</p>
        </div>
        
        <div class="d-none flex-column h-100" id="chat-active">
            <div class="chat-header">
                <div class="active-user-info">
                    <div class="contact-avatar" id="active-avatar">C</div>
                    <div>
                        <span class="active-user-name" id="active-name">Partner Company</span>
                        <span class="active-user-role" id="active-role">Company</span>
                    </div>
                </div>
            </div>
...
function initNotifications() {
    ...
    if (Notification.permission !== "granted" && Notification.permission !== "denied") {
        // Show permission request banner inside sidebar header
...
function showNotification(senderName, messageText) {
    if (Notification.permission === "granted" && document.visibilityState === "hidden") {
        new Notification("GreenBridge Alert", {
            ...
            icon: "https://cdn-icons-png.flaticon.com/512/3135/3135715.png" // Professional icon
        });
    }
}
...
function renderContacts() {
    ...
    contactsList.forEach(c => {
        ...
        // Dynamic status badge color based on role
        let badgeColor = '#4caf78'; // green for student
        if (c.type === 'company') badgeColor = '#c9952a'; // gold for company
        if (c.type === 'admin') badgeColor = '#1a3a24'; // dark green for admin
    ...
    fetch(`../api/chat_handler.php?action=get_messages&contact_id=${activeContactId}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                // Track if new messages arrived to fire notification alerts
...
document.getElementById('chat-send-form').addEventListener('submit', function(e) {
    ...
    // Optimistic insert to UI for supreme premium snappiness!
    const messageContainer = document.getElementById('chat-messages');
